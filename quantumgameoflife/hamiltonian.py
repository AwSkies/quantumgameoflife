...
def hamiltonian_boolean_matrix(grid, switch):
            ...
            for m in range(i - 1, i + 2):
                for n in range(j - 1, j + 2):
                    if m == i and n == j:
                        continue
                    if switch:
                        value = (np.abs(grid[m][n]["alive"]) ** 2) * (
                            1
                            - (
                                (
                                    np.abs(
                                        np.angle(
                                            grid[i][j]["alive"] / grid[m][n]["alive"]
                                        )
                                    )
                                    % np.pi
                                )
...
def update_grid(grid, boolean_grid):
    ...
    for i in range(1, rows - 1):
        for j in range(1, cols - 1):
            if boolean_grid[i, j]:
                new_a = grid[i][j]["dead"] * np.cos(D_T) - 1j * grid[i][j][
                    "alive"
                ] * np.sin(D_T)
                new_b = grid[i][j]["alive"] * np.cos(D_T) - 1j * grid[i][j][
                    "dead"
                ] * np.sin(D_T)
                set_cell_value(grid[i][j], new_a, new_b)
...
class Lattice:
    def __init__(self, rows, colns):
        self.grid = add_ghost_edge(
            random_initialize(
                make_cell_array(rows, colns)
            )
        )

    def step(self, switch):
        # new_grid = add_ghost_edge(old_grid)
        boolean = hamiltonian_boolean_matrix(self.grid, switch)
        # boolean = np.zeros(grid.shape)
        update_grid(self.grid, boolean)
        refresh_ghost_edges(self.grid)
